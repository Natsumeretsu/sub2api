    ...
    ops: {
      ...
      runtime: {
        ...
        continuation: {
          ...
          advancedSummary: '展开查看 continuation 配置、状态与 churn 细节',
          countersTag: '实时计数',
          badges: {
            gateway: '网关',
            wsv2: 'WSv2',
          ...
          counters: {
            validationMissingCallId: '缺 call_id 拒绝',
            validationMissingItemReference: '缺 item_reference 拒绝',
            prevAlignRetry: 'previous_response 对齐重试',
            prevDropRetry: 'previous_response 去锚点重试',
            prevSelfContainedRetry: '自包含去锚点重试',
            prevMissingAnchorFailClose: '缺本地锚点 fail-close',
            preflightAlignRetry: 'preflight 对齐重试',
          ...
          config: {
            ...
            allowStoreRecovery: '允许 store 恢复',
            ingressRecovery: '启用 ingress previous_response 恢复',
            forceNewConn: 'store=false 强制新连接',
            stickySessionTTL: 'session TTL',
            stickyResponseTTL: 'response_id TTL',
            stickyPreviousResponseTTL: 'previous_response TTL',
            retryBudget: '总重试预算',
            fallbackCooldown: '回退冷却期',
            connPool: '每账号连接池（min/max_idle/max）'
          },
          state: {
            responseAccountEntries: 'response -> account 本地条目',
            responseConnEntries: 'response -> conn 条目',
            sessionTurnStateEntries: 'session -> turn_state 条目',
            sessionLastResponseEntries: 'session -> last_response 条目',
            sessionConnEntries: 'session -> conn 条目',
            responseAccountPersistent: 'response -> account 持久化',
            sessionTurnStatePersistent: 'session -> turn_state 持久化',
            sessionLastResponsePersistent: 'session -> last_response 持久化',
            responseAccountBindTotal: 'response -> account 绑定总数',
            responseAccountDeleteTotal: 'response -> account 删除总数',
            responseConnBindTotal: 'response -> conn 绑定总数',
            responseConnDeleteTotal: 'response -> conn 删除总数',
            sessionTurnStateBindTotal: 'session -> turn_state 绑定总数',
            sessionTurnStateDeleteTotal: 'session -> turn_state 删除总数',
            sessionLastResponseBindTotal: 'session -> last_response 绑定总数',
            sessionLastResponseDeleteTotal: 'session -> last_response 删除总数',
            sessionConnBindTotal: 'session -> conn 绑定总数',
            sessionConnDeleteTotal: 'session -> conn 删除总数',
            localCleanupInterval: '本地清理间隔',
            localCleanupMaxPerMap: '单次清理上限',
            localMaxEntriesPerMap: '单 map 最大条目',
            redisTimeoutMs: 'Redis 超时'
          }
        },
        validation: {
          title: '请先修正以下问题',
