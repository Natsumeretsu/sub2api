...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledFunctionCallOutputRejectsWhenLastResponseIDMissing(t *testing.T) {
	...
	case serverErr := <-serverErrCh:
		...
		require.ErrorAs(t, serverErr, &closeErr)
		require.Equal(t, coderws.StatusPolicyViolation, closeErr.StatusCode())
		require.Contains(t, closeErr.Reason(), "function_call_output requires item_reference ids matching each call_id")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket 结束超时")
	}
	...
	require.False(t, gjson.Get(requestToJSONString(captureConn.writes[0]), "previous_response_id").Exists(), "首轮普通请求不应被补齐 previous_response_id")
}

func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledFunctionCallOutputAutoAttachPreviousResponseIDFromSessionState(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_auto_prev_session_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"response.completed","response":{"id":"resp_auto_prev_session_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	captureDialer := &openAIWSCaptureDialer{conn: captureConn}
	pool := newOpenAIWSConnPool(cfg)
	...
	account := &Account{
		ID:          145,
		Name:        "openai-ingress-fco-auto-prev-session",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	connectClient := func() *coderws.Conn {
		dialCtx, cancelDial := context.WithTimeout(context.Background(), 3*time.Second)
		clientConn, _, err := coderws.Dial(dialCtx, "ws"+strings.TrimPrefix(wsServer.URL, "http"), nil)
		cancelDial()
		require.NoError(t, err)
	...
	writeMessage(firstClient, `{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"prompt_cache_key":"pk_auto_prev_session","input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage(firstClient)
	require.Equal(t, "resp_auto_prev_session_1", gjson.GetBytes(firstTurn, "response.id").String())
	require.NoError(t, firstClient.Close(coderws.StatusNormalClosure, "done"))

	secondClient := connectClient()
	defer func() {
		_ = secondClient.CloseNow()
	}()
	writeMessage(secondClient, `{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"prompt_cache_key":"pk_auto_prev_session","input":[{"type":"function_call_output","call_id":"call_auto_session_1","output":"ok"}]}`)
	secondTurn := readMessage(secondClient)
	require.Equal(t, "resp_auto_prev_session_2", gjson.GetBytes(secondTurn, "response.id").String())
	...
	for i := 0; i < 2; i++ {
		...
		case serverErr := <-serverErrCh:
			require.NoError(t, serverErr)
		case <-time.After(5 * time.Second):
			t.Fatal("等待 ingress websocket 会话结束超时")
		}
	}

	require.Len(t, captureConn.writes, 2)
	require.Equal(t, "resp_auto_prev_session_1", gjson.Get(requestToJSONString(captureConn.writes[1]), "previous_response_id").String(), "跨客户端会话时应从 session 状态回填上一轮 response.id")
}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledPreflightPingFailClearsSessionConn(t *testing.T) {
	gin.SetMode(gin.TestMode)
	prevPreflightPingIdle := openAIWSIngressPreflightPingIdle
	openAIWSIngressPreflightPingIdle = 0
	defer func() {
	...
	firstConn := &openAIWSPreflightFailConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_cleanup_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
	...
	account := &Account{
		ID:          117,
		Name:        "openai-ingress-preflight-ping-session-cleanup",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	firstPayload := `{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"prompt_cache_key":"pk_ping_cleanup","input":[{"type":"input_text","text":"hello"}]}`
	secondPayload := `{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"prompt_cache_key":"pk_ping_cleanup","input":[{"type":"input_text","text":"world"}]}`

	hashRecorder := httptest.NewRecorder()
	hashCtx, _ := gin.CreateTestContext(hashRecorder)
	hashCtx.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", nil)
	sessionHash := svc.GenerateSessionHash(hashCtx, []byte(firstPayload))
	...
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_ping_cleanup_1", gjson.GetBytes(firstTurn, "response.id").String())

	stateStore := svc.getOpenAIWSStateStore()
	require.NotNil(t, stateStore)
	var boundConnID string
	require.Eventually(t, func() bool {
		var ok bool
		boundConnID, ok = stateStore.GetSessionConn(0, sessionHash)
		return ok
	}, time.Second, 20*time.Millisecond, "首轮成功后应绑定 session conn")
	require.NotEmpty(t, boundConnID)
	...
	case serverErr := <-serverErrCh:
		require.Error(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket preflight ping fail 清理 session conn 超时")
	}

	_, ok := stateStore.GetSessionConn(0, sessionHash)
	require.False(t, ok, "旧连接被判坏后应清理 session conn 绑定")
	require.GreaterOrEqual(t, dialer.DialCount(), 2, "ping 失败后仍应尝试重新建连")
	require.GreaterOrEqual(t, firstConn.PingCount(), 1, "第二轮前应执行 preflight ping")
}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledFunctionCallOutputPreflightPingFailAlignsPrevID(t *testing.T) {
	...
	firstConn := &openAIWSPreflightFailConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_fco_align_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_fco_align_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          143,
		Name:        "openai-ingress-preflight-ping-fco-align",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_ping_fco_align_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_stale_external","input":[{"type":"function_call_output","call_id":"call_ping_align_1","output":"ok"}]}`)
	secondTurn := readMessage()
	require.Equal(t, "resp_turn_ping_fco_align_2", gjson.GetBytes(secondTurn, "response.id").String())
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket function_call_output preflight ping 恢复超时")
	}

	require.Equal(t, 2, dialer.DialCount(), "function_call_output stale previous_response_id 遇到 preflight ping 失败后应换连并对齐锚点")
	require.Equal(t, 1, firstConn.WriteCount(), "preflight ping 失败后不应继续在旧连接写第二轮")
	require.GreaterOrEqual(t, firstConn.PingCount(), 1, "第二轮前应执行 preflight ping")
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledFunctionCallOutputPreflightPingFailClosesWhenLocalAnchorIsStillStale(t *testing.T) {
	gin.SetMode(gin.TestMode)
	resetOpenAIWSContinuationStatsForTest()
	prevPreflightPingIdle := openAIWSIngressPreflightPingIdle
	openAIWSIngressPreflightPingIdle = 0
	...
	firstConn := &openAIWSPreflightFailConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_fco_stale_local_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn},
	...
	account := &Account{
		ID:          249,
		Name:        "openai-ingress-fco-preflight-stale-local",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_ping_fco_stale_local_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_turn_ping_fco_stale_local_1","input":[{"type":"function_call_output","call_id":"call_ping_stale_local_1","output":"ok"}]}`)
	select {
	case serverErr := <-serverErrCh:
		require.Error(t, serverErr)
		var closeErr *OpenAIWSClientCloseError
		require.ErrorAs(t, serverErr, &closeErr)
		require.Equal(t, coderws.StatusPolicyViolation, closeErr.StatusCode())
		require.Contains(t, closeErr.Reason(), "matched the local continuation anchor")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket preflight stale local anchor fail-close 超时")
	}

	require.Equal(t, 1, dialer.DialCount(), "本地锚点已对齐但仍 stale 且 tool turn 不自包含时不应在 ping 失败后继续重试")
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledFunctionCallOutputPreflightPingFailClosesWithoutLocalAnchor(t *testing.T) {
	...
	account := &Account{
		ID:          144,
		Name:        "openai-ingress-fco-preflight-fail-close",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	firstTurn := readMessage()
	require.Equal(t, "response.completed", gjson.GetBytes(firstTurn, "type").String())
	require.Empty(t, gjson.GetBytes(firstTurn, "response.id").String(), "首轮响应不返回 response.id，模拟无本地续链锚点")

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_stale_external","input":[{"type":"function_call_output","call_id":"call_ping_fail_close_1","output":"ok"}]}`)
	select {
	case serverErr := <-serverErrCh:
		require.Error(t, serverErr)
		var closeErr *OpenAIWSClientCloseError
		require.ErrorAs(t, serverErr, &closeErr)
		require.Equal(t, coderws.StatusPolicyViolation, closeErr.StatusCode())
		require.Contains(t, closeErr.Reason(), "cannot be recovered without a local continuation anchor")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket function_call_output preflight fail-close 超时")
	}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledFunctionCallOutputPreflightPingFailDropsPrevIDWhenSelfContained(t *testing.T) {
	...
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_fco_self_contained_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          244,
		Name:        "openai-ingress-fco-preflight-self-contained",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_stale_external","input":[{"type":"function_call","call_id":"call_ping_self_contained_1","name":"demo","arguments":"{}"},{"type":"function_call_output","call_id":"call_ping_self_contained_1","output":"ok"}]}`)
	secondTurn := readMessage()
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket self-contained preflight 恢复结束超时")
	}

	require.Equal(t, 2, dialer.DialCount(), "自包含工具上下文应在 preflight ping 失败后切到新连接重试")
	require.GreaterOrEqual(t, firstConn.PingCount(), 1, "第二轮前仍应执行 preflight ping")
	...
	require.Len(t, secondWrites, 1)
	secondWrite := requestToJSONString(secondWrites[0])
	require.False(t, gjson.Get(secondWrite, "previous_response_id").Exists(), "自包含工具上下文重试时应移除 previous_response_id")
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PreviousResponseNotFoundFailClosesFunctionCallOutputWithoutLocalAnchor(t *testing.T) {
	...
	firstConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"previous_response_not_found","message":"stale fco anchor"}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn},
	...
	account := &Account{
		ID:          145,
		Name:        "openai-ingress-fco-prev-not-found-fail-close",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_stale_external","input":[{"type":"function_call_output","call_id":"call_prev_fail_close_1","output":"ok"}]}`)
	select {
	case serverErr := <-serverErrCh:
		require.Error(t, serverErr)
		var closeErr *OpenAIWSClientCloseError
		...
		require.Contains(t, closeErr.Reason(), "cannot be recovered without a local continuation anchor")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket function_call_output previous_response_not_found fail-close 超时")
	}

	require.Equal(t, 1, dialer.DialCount(), "无本地锚点时不应在 previous_response_not_found 后继续重试")
	firstConn.mu.Lock()
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PreviousResponseNotFoundRecoversFunctionCallOutputByAligningPrevID(t *testing.T) {
	...
	firstConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_prev_fco_align_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"previous_response_not_found","message":"stale fco anchor"}}`),
		},
	}
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_prev_fco_align_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          121,
		Name:        "openai-ingress-prev-recovery-fco-align",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"prompt_cache_key":"pk_prev_fco_align","input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_prev_fco_align_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"prompt_cache_key":"pk_prev_fco_align","previous_response_id":"resp_stale_external","input":[{"type":"function_call_output","call_id":"call_align_1","output":"ok"}]}`)
	secondTurn := readMessage()
	require.Equal(t, "resp_turn_prev_fco_align_2", gjson.GetBytes(secondTurn, "response.id").String())
	...
	require.Equal(t, 2, dialer.DialCount(), "function_call_output stale previous_response_id 应触发一次对齐后重试")

	firstConn.mu.Lock()
	firstWrites := append([]map[string]any(nil), firstConn.writes...)
	firstConn.mu.Unlock()
	require.Len(t, firstWrites, 2)
	require.Equal(t, "resp_stale_external", gjson.Get(requestToJSONString(firstWrites[1]), "previous_response_id").String(), "首轮发送应保留客户端原始 previous_response_id")
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PreviousResponseNotFoundFailClosesFunctionCallOutputWhenLocalAnchorIsStillStale(t *testing.T) {
	gin.SetMode(gin.TestMode)
	resetOpenAIWSContinuationStatsForTest()
	...
	firstConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_prev_fco_stale_local_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"previous_response_not_found","message":"stale local fco anchor"}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn},
	...
	account := &Account{
		ID:          247,
		Name:        "openai-ingress-prev-recovery-stale-local-anchor",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_prev_fco_stale_local_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_turn_prev_fco_stale_local_1","input":[{"type":"function_call_output","call_id":"call_prev_stale_local_1","output":"ok"}]}`)
	select {
	case serverErr := <-serverErrCh:
		require.Error(t, serverErr)
		var closeErr *OpenAIWSClientCloseError
		...
		require.Contains(t, closeErr.Reason(), "matched the local continuation anchor")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket stale local anchor fail-close 超时")
	}

	require.Equal(t, 1, dialer.DialCount(), "本地锚点已对齐但仍 stale 且 tool turn 不自包含时不应继续重试")
	firstConn.mu.Lock()
	firstWrites := append([]map[string]any(nil), firstConn.writes...)
	firstConn.mu.Unlock()
	require.Len(t, firstWrites, 2)
	require.Equal(t, "resp_turn_prev_fco_stale_local_1", gjson.Get(requestToJSONString(firstWrites[1]), "previous_response_id").String())
	stats := OpenAIWSContinuationStats()
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PreviousResponseNotFoundDropsPrevIDWhenAlignedFunctionCallOutputIsSelfContained(t *testing.T) {
	gin.SetMode(gin.TestMode)
	resetOpenAIWSContinuationStatsForTest()
	...
	firstConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_prev_fco_self_contained_local_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"previous_response_not_found","message":"stale aligned self contained anchor"}}`),
		},
	}
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_prev_fco_self_contained_local_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          248,
		Name:        "openai-ingress-prev-recovery-aligned-self-contained",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_prev_fco_self_contained_local_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_turn_prev_fco_self_contained_local_1","input":[{"type":"function_call","call_id":"call_prev_self_contained_local_1","name":"demo","arguments":"{}"},{"type":"function_call_output","call_id":"call_prev_self_contained_local_1","output":"ok"}]}`)
	secondTurn := readMessage()
	require.Equal(t, "resp_turn_prev_fco_self_contained_local_2", gjson.GetBytes(secondTurn, "response.id").String())
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket stale aligned self-contained 恢复结束超时")
	}

	require.Equal(t, 2, dialer.DialCount(), "本地锚点仍 stale 但请求自包含时应允许单次 drop previous_response_id 重试")
	secondConn.mu.Lock()
	secondWrites := append([]map[string]any(nil), secondConn.writes...)
	secondConn.mu.Unlock()
	require.Len(t, secondWrites, 1)
	...
	require.Contains(t, secondWrite, `"type":"function_call"`)
	require.Contains(t, secondWrite, `"call_id":"call_prev_self_contained_local_1"`)
	require.Contains(t, secondWrite, `"type":"function_call_output"`)
	stats := OpenAIWSContinuationStats()
	...
	require.Zero(t, stats.PrevNotFoundFailClosedStaleAnchorTotal)
	require.Zero(t, stats.PrevNotFoundAlignRetryTotal)
}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PreviousResponseNotFoundDropsPrevIDWhenFunctionCallOutputIsSelfContained(t *testing.T) {
	...
	firstConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"previous_response_not_found","message":"stale self contained anchor"}}`),
		},
	}
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_prev_fco_self_contained_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          245,
		Name:        "openai-ingress-prev-recovery-self-contained",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_stale_external","input":[{"type":"function_call","call_id":"call_prev_self_contained_1","name":"demo","arguments":"{}"},{"type":"function_call_output","call_id":"call_prev_self_contained_1","output":"ok"}]}`)
	secondTurn := readMessage()
	require.Equal(t, "response.completed", gjson.GetBytes(secondTurn, "type").String())
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket previous_response_not_found self-contained 恢复结束超时")
	}

	require.Equal(t, 2, dialer.DialCount(), "无本地锚点但请求自包含工具上下文时应切到新连接重试")
	...
	require.Len(t, secondWrites, 1)
	secondWrite := requestToJSONString(secondWrites[0])
	require.False(t, gjson.Get(secondWrite, "previous_response_id").Exists(), "自包含工具上下文恢复重试时应移除 previous_response_id")
	require.Contains(t, secondWrite, `"type":"function_call"`)
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_RejectsFunctionCallOutputWithoutLocalContext(t *testing.T) {
	...
	account := &Account{
		ID:          146,
		Name:        "openai-ingress-fco-validation",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"function_call_output","call_id":"call_missing_ref","output":"ok"}]}`))
	cancelWrite()
	require.NoError(t, err)
	...
	require.Equal(t, 0, captureDialer.DialCount(), "本地 function_call_output 上下文校验应在上游拨号前失败")
	require.Empty(t, captureConn.writes, "本地校验失败时不应向上游发送任何 payload")
}
