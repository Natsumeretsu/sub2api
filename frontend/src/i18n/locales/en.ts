    ...
    ops: {
      ...
      runtime: {
        ...
        continuation: {
          ...
          advancedSummary: 'Show continuation config, state, and churn details',
          countersTag: 'Live counter',
          badges: {
            gateway: 'Gateway',
            wsv2: 'WSv2',
          ...
          sections: {
            config: 'Strategy and budget',
            stateEntries: 'Local state entries',
            persistence: 'Persistence support',
            churn: 'Bind / cleanup totals',
            limits: 'Local cleanup and capacity'
          },
          counters: {
            validationMissingCallId: 'Rejected missing call_id',
            validationMissingItemReference: 'Rejected missing item_reference',
            prevAlignRetry: 'previous_response align retries',
            prevDropRetry: 'previous_response drop retries',
            prevSelfContainedRetry: 'self-contained drop retries',
            prevMissingAnchorFailClose: 'missing local anchor fail-close',
            preflightAlignRetry: 'preflight align retries',
          ...
          config: {
            ...
            allowStoreRecovery: 'Allow store recovery',
            ingressRecovery: 'Ingress previous_response recovery',
            forceNewConn: 'Force new conn when store=false',
            stickySessionTTL: 'Session TTL',
            stickyResponseTTL: 'response_id TTL',
            stickyPreviousResponseTTL: 'previous_response TTL',
            retryBudget: 'Total retry budget',
            fallbackCooldown: 'Fallback cooldown',
            connPool: 'Per-account pool (min/max_idle/max)'
          },
          state: {
            responseAccountEntries: 'response -> account local entries',
            responseConnEntries: 'response -> conn entries',
            sessionTurnStateEntries: 'session -> turn_state entries',
            sessionLastResponseEntries: 'session -> last_response entries',
            sessionConnEntries: 'session -> conn entries',
            responseAccountPersistent: 'response -> account persistent',
            sessionTurnStatePersistent: 'session -> turn_state persistent',
            sessionLastResponsePersistent: 'session -> last_response persistent',
            responseAccountBindTotal: 'response -> account bind total',
            responseAccountDeleteTotal: 'response -> account delete total',
            responseConnBindTotal: 'response -> conn bind total',
            responseConnDeleteTotal: 'response -> conn delete total',
            sessionTurnStateBindTotal: 'session -> turn_state bind total',
            sessionTurnStateDeleteTotal: 'session -> turn_state delete total',
            sessionLastResponseBindTotal: 'session -> last_response bind total',
            sessionLastResponseDeleteTotal: 'session -> last_response delete total',
            sessionConnBindTotal: 'session -> conn bind total',
            sessionConnDeleteTotal: 'session -> conn delete total',
            localCleanupInterval: 'Local cleanup interval',
            localCleanupMaxPerMap: 'Cleanup max per map',
            localMaxEntriesPerMap: 'Local max entries per map',
            redisTimeoutMs: 'Redis timeout'
          }
        },
        validation: {
          title: 'Please fix the following issues',
